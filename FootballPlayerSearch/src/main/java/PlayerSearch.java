...
public class PlayerSearch extends WebScraper {
    public static boolean searchPrompt(String searchBy) {
        ...
        while (true) {
            ...
            String searchByName = scanner.nextLine().toLowerCase();

            if (searchByName.equals("yes")) {
                break;
            } else if (searchByName.equals("no")) {
                return false;
            } else {
    ...
    private static void searchPlayersByPosition() throws IOException {
        ...
        if (!(searchForPosition)) {
            ...
            String positionSearch = scanner.nextLine().toLowerCase();
            if(positionSearch.equals("midfielder")) {
                positionSearch = "mf";
            }
            if(positionSearch.equals("forward")){
                positionSearch = "fw";
            }
            if (positionSearch.equals("defender")){
                positionSearch = "df";
            }
            if (positionSearch.equals("goalkeeper")) {
                positionSearch = "gk";
            }
            System.out.println("Fetching player results...\n");
            List<PlayerStats> playerStatsList = getAllPlayerStats();
    ...
    public static void searchPlayersByNationality() throws IOException {
            ...
            if (!searchForNationality) {
                ...
                String nationalitySearch = scanner.nextLine().trim().toLowerCase();
                ...
                for (PlayerStats playerStats : playerStatsList) {
                    if (playerStats.getPlayerNationality().equalsIgnoreCase(nationalitySearch)) {
                        resultCount++;
                        searchResults.add(playerStats);
                    }
                }
        if (resultCount == 0) {
            System.out.println("No player with the position '" + nationalitySearch.substring(0, 1).toUpperCase() + nationalitySearch.substring(1).toLowerCase() + "' found.");
        } else if (resultCount == 1) {
            System.out.println("Only one result found that matches your search:\n");
            PlayerStats onlyResult = searchResults.get(0);
            PlayerStats.displayAdvancedPlayerStats(onlyResult);
    ...
    public static void searchPlayersByClub() {
